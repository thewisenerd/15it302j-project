...
var errexit = function(res, error) {
  res.send({
    status: error[0],
    data: {
      "msg": error[1]
    ...
    if (results.length == 0) {
      ...
      if (method == 'post' && (rq.pass || rq.displayname || rq.displaydesc || rq.email)) {
        ...
        if (!rq.pass || !rq.displayname || !rq.displaydesc || !rq.email) {
          send(res, config.e.E_WRONG_PARAMETERS, {
        ...
        query(res, mysql.format(
          ...
          send(res, config.e.E_OK, results);
        });
      } else {
      /* this is an empty query, not trying to make a new user. */
        send(res, config.e.E_WRONG_PARAMETERS, {
  ...
  if (user.key) {
    ...
    query(res, q, function(err, results, fields) {
      ...
      if (results.changedRows == 1) {
        ...
        errexit(res, config.e.E_KEY_FAILURE);
      }
    });
  } else { // user.key
